...
功能概要：
- 支援載入 ResNet50 預訓練權重並進行遷移學習。
- stratified 模式與 folders 模式資料讀取。

# 執行範例：
python train_resnet50_RadImageNet.py \
  --weights-path "weights/ResNet50.pt" \
  --freeze-base \
  --data-dir "data/alzheimer" \
  --split-mode "folders" \
  --val-ratio 0.1 \
  --test-ratio 0.1 \
  --lr 0.001 \
  ...
  --epochs 20 \
  --device cuda \
  --output-dir "outputs/resnet50_radimagenet"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--weights-path",
        type=str,
        default=Path("weights_RadImageNet_pytorch/ResNet50.pt"),
        help="預訓練權重的路徑 (.pth 或 .pt)。若不提供則從頭訓練。",
    )
    parser.add_argument(
        "--freeze-base",
        action="store_true",
        help="是否凍結 ResNet50 的基礎卷積層，僅訓練最後的全連接層。",
    )
...
class BasicImageTransform:
    ...
    def __call__(self, image: Image.Image) -> torch.Tensor:
        ...
        # ResNet50 標準化參數 (ImageNet/RadImageNet 通用常態化設定)
